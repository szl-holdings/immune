...
import FOUNDATION_SOURCES from "../data/foundations.sources.json";

const GH = "https://github.com/szl-holdings";

// Maturity is the honesty spine of this panel: we NEVER call a conjecture a
// theorem. PROVEN = machine-checked in Lean 4 (zero-sorry); CONJECTURE =
// advisory, explicitly unproven; METHOD = a real engineering/measurement
// recipe, not a proof.
type Maturity = "PROVEN" | "CONJECTURE" | "METHOD";

// The machine-checkable source of truth. Each served card MUST have an entry
// here, and `scripts/check-foundations-corpus.mjs` fails the build if any
// `anchor` is not present verbatim in the committed corpus/<repo>.md snapshot.
...
// The human-facing content, keyed by the same id. Kept in the component because
// the formulas are rendered JSX (Unicode math, no KaTeX dependency).
...
const sub = (s: ReactNode) => <sub className="text-[0.6em]">{s}</sub>;

// Every entry below is transcribed VERBATIM from its canonical szl-holdings
// repository (linked). Nothing here is invented — this is the "canonical
// corpus" the served surfaces must match, and the honesty guard enforces it.
const CONTENT: Record<string, FoundationContent> = {
  lambda: {
    title: "Λ — Governance aggregator",
    subtitle: "Λ (Lambda-Spine): weighted geometric mean of axis scores",
    formula: (
      <>
        Λ(x) = <span className="text-primary">∏{sub("i")}</span> x{sub("i")}
        <sup className="text-[0.65em]">w{sub("i")}</sup>
      </>
    ),
    where: (
      <>
        ∑{sub("i")} w{sub("i")} = 1 &nbsp;·&nbsp; w{sub("i")} &gt; 0 &nbsp;·&nbsp;
        x{sub("i")} ∈ [0,1]
      </>
    ),
  ...
  axioms: {
    ...
    formula: (
      <div className="flex flex-col gap-1 text-left text-sm sm:text-base">
        ...
        <span>
          <span className="text-primary">A4</span> bounded — Λ(x) ≤ max{sub("i")} x
          {sub("i")}
        </span>
      </div>
    ),
  ...
  theoremU: {
    ...
    subtitle: "Uniqueness modulo ≈Λ under the Identifiability Assumptions",
    formula: (
      <>
        IA ⊢ Λ{sub("1")} <span className="text-primary">≈Λ</span> Λ{sub("2")}
      </>
    ),
    where: (
      <>
        strict Λ{sub("1")} = Λ{sub("2")} only under Anchored / Normalized
      </>
    ),
    note: "Axiom-free, zero-sorry (TheoremU_LambdaUnique): any two IA-solutions coincide modulo the audit-invariant ≈Λ. This is CONDITIONAL — unconditional Λ uniqueness under bare A1–A5 stays Conjecture 1, machine-checked FALSE as stated. We never write 'Λ is unique' without this qualifier.",
  },
  lutar: {
    ...
    formula: (
      <>
        ⊢ locked_count_eight : <span className="text-primary">|locked| = 8</span>
      </>
    ),
  ...
  neyman: {
    title: "Neyman–Pearson optimality (IMMUNE detection)",
    subtitle: "The likelihood-ratio tripwire is the most powerful test at fixed false-alarm rate",
    formula: (
      <>
        reject H{sub("0")} ⟺{" "}
        <span className="text-primary">
          L(x) = p{sub("1")}(x) ⁄ p{sub("0")}(x) ≥ k
        </span>
      </>
    ),
    where: (
      <>
        most powerful test at false-alarm level α = P{sub("0")}(L ≥ k)
      </>
    ),
    note: "IMMUNE's likelihood-ratio detector is provably the most powerful test at any fixed false-alarm rate. Kernel-verified in Lean as CF-5 'immune Neyman–Pearson optimality' (Wave 11, EXPERIMENTAL·CI-green tier — verified, but never folded into the locked-8).",
  },
  pinsker: {
    title: "Binary Pinsker inequality",
    subtitle: "A distribution can't drift far without the KL alarm firing",
    formula: (
      <>
        <span className="text-primary">2(p − q)² ≤ KL(p ‖ q)</span>
      </>
    ),
    where: <>Bernoulli(p) vs Bernoulli(q) · total variation ≤ √(KL ⁄ 2)</>,
    note: "binary_pinsker — axiom-free, zero-sorry (Wave 17). Lower-bounds KL divergence by total variation, the quantitative backbone of drift/anomaly detection. Generalizes to k classes via multiclass_pinsker (data-processing reduction).",
  },
  norm: {
    ...
    note: "Computed in float32 for numerical stability, verified against PyTorch's own reference. Governed mode emits content-addressed, SHA3-256 hash-chained receipts of each call.",
  },
  khipu: {
  ...
  yarqa: {
    title: "yarqa — plug-flow compartmentalization (CFD)",
    subtitle: "Velocity-aligned reduction of a CFD field + reproducible receipts",
    formula: (
      <>
        grow ⟸{" "}
        <span className="text-primary">dot(û_seed, û_k) ≥ threshold</span>
      </>
    ),
    where: <>compartmentalize_with_receipt(mesh) → labels, receipt</>,
    note: "A CFD / reactor-engineering METHOD — NOT a PINN and NOT a locked theorem. Each run emits a replayable, Sigstore-keyless-signable provenance receipt: an integrity + reproducibility guarantee, never a proof of correctness.",
  },
};

const sources = FOUNDATION_SOURCES as FoundationSource[];
...
function FoundationCard({
  ...
  src: FoundationSource;
  content: FoundationContent;
  index: number;
}) {
  const m = MATURITY[src.maturity];
  const MIcon = m.Icon;
  return (
    <motion.a
      href={`${GH}/${src.repo}`}
      ...
      data-testid={`card-formula-${src.id}`}
      initial={{ opacity: 0, y: 16 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: "-40px" }}
      transition={{ delay: index * 0.06, type: "spring", stiffness: 200, damping: 24 }}
      className="group flex flex-col gap-3 min-w-0 rounded-sm border border-border/50 bg-black/40 p-4 sm:p-5 hover:border-primary/50 transition-colors"
    >
      ...
      <div className="rounded-sm border border-primary/15 bg-black/50 px-3 py-4 text-center overflow-hidden">
        <div className="font-mono text-base sm:text-xl text-foreground break-words [overflow-wrap:anywhere]">
          {content.formula}
        </div>
        {content.where && (
          <div className="font-mono text-[11px] sm:text-xs text-primary/60 mt-2 break-words [overflow-wrap:anywhere]">
            {content.where}
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {sources.map((src, i) => {
          const content = CONTENT[src.id];
          if (!content) return null;
          return (
            <FoundationCard key={src.id} src={src} content={content} index={i} />
          );
        })}
      </div>
      ...
      <p className="font-mono text-[10px] text-muted-foreground/60 leading-relaxed border-t border-border/40 pt-4">
        ...
        papers). Each served formula is checked against a committed snapshot of the
        canonical corpus (<code className="text-secondary/70">corpus/</code>) by an
        automated honesty guard — a formula that does not exist in that corpus fails the
        build. Overclaiming is treated as a failure, not a feature.
      </p>
    </div>
  );
}
